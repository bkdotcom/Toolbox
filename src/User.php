...
class User
{
    ...
    protected $authVia = null;      // cookie, form, session, social
    ...
    public function __construct($db, $cfg = array())
    {
        ...
        $this->cfg = ArrayUtil::mergeDeep($cfgDefault, $cfg);
        $this->db = $db;
        Session::startIf();
        $this->resume();
    ...
    public function datetimeLocal($datetime = null, $format = null, $timezone = null)
    {
        ...
        $tzServerString = \date_default_timezone_get();
        if (empty($timezone)) {
        ...
        $datetime  = new \DateTime($datetime, $tzServer);
        $datetime->setTimezone($tzUser);
        $str = $datetime->format($format);
        $this->debug->groupEnd($str);
        return $str;
    }
    ...
    public function update($vals, $userid = null)
    {
        ...
        if (!empty($vals)) {
            ...
            if ($result === false) {
                ...
                ArrayUtil::path($_SESSION, $this->cfg['sessionPath'], $this->user);
            }
        }
        if ($return['success'] && !empty($valsAuth)) {
            if (isset($valsAuth['password'])) {
                $valsAuth['password'] = $this->hashPassword($valsAuth['password']);
                $this->log('pwChange', $userid);
            }
            // try update first
            $result = $this->db->rowMan('user_auth', $valsAuth, array('userid' => $userid));
            $this->debug->log('result', $result);
            if ($result === 0) {
                // no rows affected -> try insert
                $valsAuth['userid'] = $userid;
                $result = $this->db->rowMan('user_auth', $valsAuth);
                $this->debug->log('result', $result);
            }
            if ($result === false) {
                $return['success'] = false;
    ...
    private function resume()
    {
        ...
        */
        if (!empty($this->user)) {
            ...
            $this->userid = $this->user['userid'];
            $this->authVia = 'session';
            $this->auth = 2;
            if ($this->user['lastactivity'] + $this->cfg['maxIdle'] < \time()) {
                $this->debug->warn('inactive');
        ...
        */
        if (empty($this->auth) && isset($_COOKIE[ $this->cfg['persistCookie']['name'] ])) {
            $persistVals = Cookie::get($this->cfg['persistCookie']['name']);
            $userid = isset($persistVals['username']) && isset($persistVals['token'])
                ? $this->verifyPersistToken($persistVals['username'], $persistVals['token'])  // drops from db if found
                : null;
            if ($userid) {
    ...
    private function verifyPassword($pwClear, $pwHash)
    {
        ...
        if ($success && \password_needs_rehash($pwHash, $hashAlgo, $hashOpts)) {
            if ($this->userid) {
                // store new hash
