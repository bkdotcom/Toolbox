...
class Inflect
{
    ...
    static public $singular = array(
        ...
        '/(vert|ind)ices$/i'        => "$1ex",
        '/^(ox)en$/i'               => "$1",
        '/(alias|business)es$/i'    => "$1",
        '/(octop|vir)i$/i'          => "$1us",
        '/(ax|bas|cris|parenthes|test|the)es$/i' => "$1is",
        // '/(shoe)s$/i'               => "$1",
        // '/(tive)s$/i'               => "$1",
        // '/(hive)s$/i'               => "$1",
        // '/(corpse)s$/i'             => "$1",
        '/(bus|ch|o|ss|sh|us|x)es$/i'   => "$1",
        '/([m|l])ice$/i'            => "$1ouse",
        '/(h|bl)ouses$/i'           => "$1ouse",
        '/(m)ovies$/i'              => "$1ovie",
        '/(s)eries$/i'              => "$1eries",
    ...
    public static function singularize($string)
    {
        ...
        if (in_array(strtolower($string), self::$notPlural)) {
            return $string;
        }
